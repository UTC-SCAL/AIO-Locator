export class MapsModel {
  ...
  addcircle(location: google.maps.LatLng): google.maps.Circle {
  ...
  cleanMap2(location: google.maps.LatLng) {
    // Empty nearby places array
    //this.nearby_places = [];
    ...
    // To remove all previous markers from the map
    this.map_places.forEach((place) => {
    if(place.forDeletion==true){
      place.marker.setMap(null);
      let index = this.map_places.indexOf(place);
      if (index > -1) {
        this.map_places.splice(index, 1);
      }
    }
    });
    this.map_places.forEach((place) => {
      place.forDeletion=true;
    });


    this.geofence.setCenter(location);
  ...
  //     position: location,
  //     map: this.map,
  //     icon: './assets/images/pedestrian.png'
  //     //MapPlace.createIcon(color)
  //   });
  //
  ...
  getplace(id:string){
	...
	addPlaceToMap(location: google.maps.LatLng,id: string, type:number = 4, color: string = '#333333') : MapPlace {
    if(this.checkfor(id)==true){
      let place = this.getplace(id);
      place.marker.setPosition(location);
      place.forDeletion=false;
    } else{
		let _map_place = new MapPlace();
    let png_String='';
    if (type == 0){
      png_String='./assets/images/pedestrian.png'
    }
    if (type == 1){
      png_String='./assets/images/vehicle.png'
    }
    if (type == 2){
      png_String='./assets/images/cyclist.png'
    }
    if (type == 3){
      png_String='./assets/images/object.png'
    }
    if (type == 4){
      png_String='./assets/images/ped_group.png'
    }
    if (type == 5){
      png_String='./assets/images/network.png'
    }
    _map_place.forDeletion = false;
    _map_place.ionID = id;
		_map_place.location = location;
		_map_place.marker = new google.maps.Marker({
      position: location,
	...
	addNearbyPlace(place_result: google.maps.places.PlaceResult) {
		let _map_place = new MapPlace();//this.addPlaceToMap(place_result.geometry.location, 5);
